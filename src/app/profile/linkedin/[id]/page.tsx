...
import { getLinkedInProfile } from '@/lib/linkedin-search';
...
export default async function LinkedInProfilePage({ params, searchParams }: PageProps) {
    const { id } = await params;
    const _searchParams = await searchParams; // Await to satisfy Next.js 16
    const decodedId = decodeURIComponent(id);
    const profile = await getLinkedInProfile(decodedId);
    ...
    const calculateExp = (exps: any[]) => {
        ...
        exps?.forEach(e => {
            if (e.starts_at?.year) {
                let end = new Date();
                if (e.ends_at?.year) end = new Date(e.ends_at.year, (e.ends_at.month || 1) - 1);
                const start = new Date(e.starts_at.year, (e.starts_at.month || 1) - 1);
                if (end > start) totalMonths += (end.getFullYear() - start.getFullYear()) * 12 + (end.getMonth() - start.getMonth());
            }
        });
    ...
    const currentRole = profile.experiences?.[0] || { title: "Open to Work", company: "N/A" };
    ...
    const analysis = generateGapAnalysis(missingSkills, profile.occupation || "Developer", skills);
    ...
    return (
        <div className="min-h-screen bg-slate-950 text-cyan-500 font-mono p-6">
            ...
            <div className="flex justify-between items-center mb-8 border-b border-cyan-900/50 pb-4">
                <Link href="/search?mode=linkedin" className="text-sm hover:text-cyan-300 transition-colors">
                    [ &lt; BACK ]
                </Link>
